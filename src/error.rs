...
pub fn get_error_and_line_for_file(
...
pub fn print_interpreter_error_for_file(
    err: RuntimeError,
    span: SourceSpan,
    file_content: &str,
    file_name: &str,
) {
    print_typechecker_error_for_file(
        TypeCheckerIssue::RuntimeError(err),
        span,
        file_content,
        file_name,
    );
...
pub fn print_typechecker_error_for_file(
    err: TypeCheckerIssue,
    span: SourceSpan,
    file_content: &str,
    file_name: &str,
) {
    ...
    match err {
        ...
        TypeCheckerIssue::FunctionReturnsMultipleTypes => {
            println!(
                "{}: different branches of the function return different types",
                Style::new().bold().paint("function returns multiple types")
            );
            is_error = false;
        }
        TypeCheckerIssue::UnreachableCodeAfterReturn => {
            println!(
                "{}: code was found after a return statement, making it unreachable",
